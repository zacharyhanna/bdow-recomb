...
def ldhelmet_find_confs(scaf, combined_fasta):
    ...
    output_base = combined_fasta[:-5]
    output = output_base + "conf"
...
def ldpop(scaf, combined_fasta, scaf_theta):
    ...
    ldpop_cmd = "~/bin/ldpop/run/ldtable.py -n 24 -th " + scaf_theta + " -rh 101,100 --approx --cores 12 >" + output
    Run(ldpop_cmd)
...
def run_pipeline(scaf_file):
    ...
    for scaf in scaf_ls:
        ...
        #ldhelmet_find_confs(scaf, combo_fasta)
        ldpop(scaf, combo_fasta, scaf_theta)
